...
import { User, VotingStates } from '@voting.poker/core';
...
const RoomDetails = ({ roomId, user, users }: RoomDetailsProps) => {
  ...
  const handleEndSession = () => {
    if (!room.state.moderator || (room.state.state !== VotingStates.Pool && room.state.state !== VotingStates.PoolVote)) {
      return;
    }
  ...
  const handleStartSession = () => {
    if (!room.state.moderator || (room.state.state !== VotingStates.Idle && room.state.state !== VotingStates.PoolResult)) {
      return;
    }
